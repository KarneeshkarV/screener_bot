...
def main() -> None:
    ...
    if client is None:
        raise SystemExit(
            "Turso not configured (set TURSO_DATABASE_URL / TURSO_AUTH_TOKEN)."
        )
    try:
        before = {
            (r["symbol"], r["market"]) for r in portfolio_store.fetch_portfolio(client)
        }
        ...
        for r in after:
            if r["symbol"] in {"AMD", "MU", "NET"}:
                print(
                    f"  {r['symbol']:5} avg={r['avg_price']} stop={r['stop_loss']} ruleset={r['ruleset']}"
                )
    finally:
        client.close()
